...
/**
 * Resolves [TeamStyle] for a given team number from the MiniGamesAPI config.
 *
 * Single source of truth — `plugins/<HostPlugin>/minigamesapi/config.yml`:
 * - Material: teamselect.teams.<n>.material  (fallback: built-in default)
 * - Color:    teamselect.teams.<n>.color     (fallback: built-in default)
 * - Name:     teamselect.teams.<n>.name      (fallback: "Команда <n>" with the configured color)
 *
 * Legacy `messages.yml: ui.teamselect.teams.<n>.name` is migrated into config.yml on
 * startup by [ru.joutak.minigames.MiniGamesCore]; it is no longer read at runtime.
 */
object TeamStyleProvider {
    ...
    fun get(teamNumber: Int): TeamStyle {
        ...
        if (!rawName.isNullOrBlank()) {
            ...
            // Match the previous default visual ("&cКоманда 1" without extra decoration).
